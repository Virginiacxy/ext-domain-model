...
class MoneyTests: XCTestCase {
  ...
  func testAddUSDtoGBP() {
    ...
    func testDoubleExtension() {
        XCTAssert(10.0.USD.currency == tenUSD.currency && 10.0.USD.amount == tenUSD.amount)
        
        let test = Money(amount: 25, currency: "YEN")
        XCTAssert(25.6.YEN.currency == test.currency && 25.6.YEN.amount == test.amount)
        
        XCTAssert(5.0.GBP.currency == fiveGBP.currency && 5.0.GBP.amount == fiveGBP.amount)
        
        XCTAssert(15.0.EUR.currency == fifteenEUR.currency && 15.0.EUR.amount == fifteenEUR.amount)
    }
    ...
    func testSubtractEURfromUSD() {
        let total = fifteenEUR.subtract(twelveUSD)
        XCTAssert(total.amount == 2)
        XCTAssert(total.currency == "USD")
    }
}
